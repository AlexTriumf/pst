...
# Function to load and execute the DLL in memory with error handling
function Invoke-DLLInMemory {
    ...
    $waitResult = [Win32]::WaitForSingleObject($hThread, 0xFFFFFFFF)

    if ($waitResult -ne 0) {
        Write-Host "[!] Thread execution failed or was interrupted. Result: $waitResult"
    } else {
        Write-Host "[+] DLL executed successfully in memory."
    }
